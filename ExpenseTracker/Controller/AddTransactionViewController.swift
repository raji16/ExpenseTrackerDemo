...
class AddTransactionViewController: UIViewController, PassDataBack {
    ...
    @IBAction func saveBtnPressed(_ sender: Any) {
        ...
        if itemToEdit == nil {
            guard let amountString = amountTxtField.text, amountString != "" else {
                showAlertMessage(message: ExpenseConstants.invalidAmount)
                return
            }
            
            guard let category = categoryTxtField.text, category != "" else {
                showAlertMessage(message: ExpenseConstants.invalidCategory)
            ...
            guard let note = noteTxtField.text, note != "" else {
                showAlertMessage(message: ExpenseConstants.invalidNote)
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        if (textField == categoryTxtField){
            ...
            if(transactionType == .expense){
                guard let categoryVC = storyboard?.instantiateViewController(withIdentifier: "ExpenseVC") as? ExpenseViewController else { return true }
                ...
                guard let categoryVC = storyboard?.instantiateViewController(withIdentifier: "IncomeVC") as? IncomeViewController else { return true }
...
extension AddTransactionViewController: UIViewControllerTransitioningDelegate {
